...
To integrate the Beam app (pipelines) running in Flink Cluster to receive the LR records from Kafka, the Beam KafkaIO() API is invoked in an Unbounded mode. The received &quot;each&quot; LR record (from Kafka broker) is stored in a Beam PCollection object as a class variable within an instance of the provided inner class that processes that particular record. To save the LR output data, Java PrintWriter is utilized to write into a flat file i.e. the Sink. Open Source Redis is integrated with the Beam app to contain the runtime required in-memory information. Redis database gets purged as per each fresh execution of the Beam app.
...
Please see pom.xml provided in this package.
